...
    使用一个名为base_q_idx的语法元素在帧头中指定量化步长，它是一个8位的固定长度编码，用于指定亮度AC系数的量化步长，有效值范围是[0, 255]。
3.亮度DC系数的量化：

    相对于base_q_idx，亮度DC系数的量化步长差值（DeltaQYDc）会被进一步信号传递。
4.色度分量的量化：

    如果存在多个色度平面，会信号传递一个标志diff_uv_delta，以指示是否应对Cb和Cr色度分量应用不同的量化步长。
5.色度DC和AC系数的量化：

    如果diff_uv_delta信号为0，只信号传递相对于base_q_idx的色度DC（DeltaQUDc）和AC（DeltaQUAc）系数的差值。
    否则，会信号传递Cb和Cr DC（DeltaQUDc和DeltaQVDc）以及AC（DeltaQUAc和DeltaQVAc）系数相对于base_q_idx的差值。
6.量化索引的计算：

    解析出的差值（DeltaQYDc、DeltaQUAc、DeltaQUDc、DeltaQVAc、DeltaQVDc）将被加到base_q_idx上，以得到量化索引。
7.量化步长的映射：

    量化索引根据适当的查找表映射到量化步长。
    对于8位、10位和12位的内部比特深度，DC系数的映射由3×256查找表Dc_Qlookup指定，AC系数的映射由3×256查找表Ac_Qlookup指定。
8.查找表的使用：

    查找表将量化索引映射到量化步长，确保不同比特深度的内部表示可以正确地量化和去量化。
9.图表说明：

  分别展示了DC和AC变换系数的量化索引到量化步长的映射关系。
![loading-ag-23839](videocodeciamges/AV1_DCAC_QUAN.png)
...
[AV1 编码标准帧间预测技术概述_av1 obmc-CSDN博客](https://yx-codec-conductor.blog.csdn.net/article/details/140374168)
...
1.参考帧选择：编码每一帧时，可以从解码帧缓冲区中选择最多两个参考帧用于帧间预测。

2.单参考帧间预测：只使用一个帧作为参考帧。

3.复合帧间预测：使用两个参考帧，并结合这两个参考帧生成的预测块来生成最终预测。

4.单向和双向复合预测：

   4.1 单向复合预测：选择的参考帧要么是都在当前帧的显示顺序之前，要么是都在当前帧之后。
   4.2双向复合预测：一个参考帧在当前帧的显示顺序之前，另一个在之后。
5.参考帧组合：

   5.1单向复合预测限制在四种组合。
   5.2双向复合预测支持所有12种可能的参考帧组合。
6.参考帧更新：编码完成后，编码器决定解码帧缓冲区中哪个参考帧需要被替换，并且在比特流中明确信号这一更新。

7.参考帧类型：解码帧缓冲区存储四种类型的参考帧：

    LAST帧：最近过去的已显示帧。
    BWD帧：将来要显示的帧。
    GOLDEN帧：较远过去的已显示帧。
    交替参考帧（ARF）：来自过去或未来的帧，可以选择不显示。
8.ARF合成：ARF可以由编码器合成，例如，可以通过沿着多个原始帧的运动轨迹进行时域滤波来生成。
