...
![X265_PU.png](videocodeciamges/SAD.png)
...
![X265_PU.png](videocodeciamges/SSE.png)
...
![X265_PU.png](videocodeciamges/MSE.png)
...
可以用以下公式表示，也可以使用后面基于MSE的写法

![X265_PU.png](videocodeciamges/PSNR_2.png)

--------------------------------------------------------------------------

![X265_PU.png](videocodeciamges/psnr.png)
...
SSIM通过模拟人类视觉系统（HVS）对亮度、对比度和结构信息的敏感性构建评价模型：

- ​**​亮度分量（Luminance）​**​：计算两帧图像的平均灰度差异，公式为：  
  l(x,y)=μx2​+μy2​+C1​2μx​μy​+C1​​，其中μx​、μy​为均值，C1​为稳定常数（通常取(0.01×255)2)

- ​**​对比度分量（Contrast）​**​：衡量图像局部对比度差异，公式为：  
  c(x,y)=σx2​+σy2​+C2​2σx​σy​+C2​​，σx​、σy​为标准差，C2​常数

- ​**​结构分量（Structure）​**​：通过协方差捕捉空间结构相似性，公式为：  
  s(x,y)=σx​σy​+C3​σxy​+C3​​，其中σxy​为协方差，C3​常取C2​/2

最终SSIM值为三者的乘积：  
SSIM(x,y)=l(x,y)⋅c(x,y)⋅s(x,y)，取值范围为0-1，值越接近1质量越高

结构相似性,范围为 0-1 越大越相似

![X265_PU.png](videocodeciamges/SSIM.png)

## VMAF

VMAF由多个指标组成.vmaf由VIF.DLM进行

若忽略具体实现细节，VMAF可近似表示为：

VMAF=w1​⋅VIF+w2​⋅DLM+w3​⋅MCTI+…+b

- w1​,w2​,w3​ 为模型学习到的权重，b 为偏置项。

### VIF( Visual Information Fidelity)

视觉信息保真度

衡量原始视频与失真视频之间的**信息损失**，基于自然场景统计模型

![X265_PU.png](videocodeciamges/VIF.PNG)

- Cref,k​ 和 Cdis,k​ 分别表示原始和失真视频的子带（小波域）系数。
- I(X;Y) 表示X和Y的互信息（Mutual Information）。
...
![X265_PU.png](videocodeciamges/DLM.PNG)

- ∇Y 表示图像的梯度（通过Sobel算子计算），ϵ 为平滑常数。
- 

## 一些缩写

sar和dar是X264/x265/ffmpeg都会使用到的概念

在X264/doc/vui.txt中 有解释

### SAR（Sample Aspect Ratio）

SAR = 像素的宽/像素的高(像素不一定是方形的)

### DAR（display aspect ratio）

即常说的4:3  16：9等，就是宽高像素数量
...
一般是帧率，对于可变帧率视频，它的值是平均帧率

### RDO

率失真优化

rate distortion optimiztion

率失真优化到底是什么意思：

在给定的码率（带宽）限制下，尽可能提供最好的画质；或者在保证一定画质的前提下，尽可能节省码率。
### RDO 的数学公式

为了让计算机自动做这个选择，RDO 引入了一个 代价函数（Cost Function） ：
...
- $D$ (Distortion) ：失真度。
- $R$ (Rate) ：消耗的比特数。
- $\lambda$ (Lagrange Multiplier) ：拉格朗日乘子，它相当于**“比特的价格”**。
  
  

工作原理 ：
编码器会遍历各种可能的编码模式，计算每种模式的 $J$ 值，最后选择 $J$ 最小的那种模式。

- 如果 $\lambda$ 很大：说明“比特很贵”，算法会倾向于选择比特数 $R$ 少的模式（哪怕画质稍差）。
- 如果 $\lambda$ 很小：说明“比特很便宜”，算法会倾向于选择画质 $D$ 好的模式（哪怕多费点比特）。

## ME

motion estimation

即运动估计，表示从参考帧中查找当前块的参考块的过程

## MC

motion compenstion

运动补偿，指的是用运动估计中找到的参考块进行预测，生成预测内容的过程         

## ROI区域​

# （Region of Interest，简称ROI）
...
不同种类的SLICE 和宏块之间是有关系的：

![X265_PU.png](videocodeciamges/H264_SLICE_MB.PNG)

某种slice中只允许出现特定的mb，如上表所示

按照I/P/B划分宏块类型 有以下分类

### I宏块

![X265_PU.png](videocodeciamges/I_MB_TYPE.PNG)

表中 ，Intra_4x4 表示使用帧内 4x4 预测，Intra_16x16 表示使用帧内 16x16 预测。当使用帧内 16x16
时，类型名称由了如下的结构组成：
 I_16x16_x_y_z
其中，x 对应于表中“帧内 16x16 的预测模式”字段的值，y 对应于表中“色度 CBP”字段的值，z 对应
于表中“亮度 CBP”的值。

- 帧内 16x16 的预测模式：当使用帧内 16x16 预测时，指定使用何种预测方式，帧内 16x16 共
  有四种预测模式，第八章中会详细介绍这些预测模式的算法。
- CodedBlockPatternLuma：指定当前宏块色度分量的 CBP，CBP（CodedBlockPattern）是指子
  宏块残差的编码方案。该变量详细语义见 coded_block_pattern 条目。
- 亮度 CBP：指定当前宏块亮度分量的 CBP，详细语义见 coded_block_pattern 条目。
  我们看到，帧内 16x16 宏块类型的 mb_type 语义原比其它宏块类型的复杂，这是因为当使用帧
  内 16x16 时，整个宏块是一个统一的整体，宏块中各子宏块、4x4 小块的预测模式信息都是相同的，
  所以可以把这些信息放入 mb_type ,以减少码流。其它宏块类型的这些信息必须在各子块中另外用句
  法元素指明

### P宏块

![X265_PU.png](videocodeciamges/P_SLICE_MB_TYPE.PNG)

Pred_L0 表示用 L0,即前向预测。如果当前宏块的 mb_type 等于 0 到 4， mb_type 的含
当 mb_type 等于 5 到 30 时， mb_type 的含义见上一个 I slice的表(复用), 用 mb_type-5 所得到的值
来进行查找。预测模式（mb_type,n）预测模式是 mb_type 的函数，n 是宏块的第 n 个分区。

### B宏块

![X265_PU.png](videocodeciamges/B_SLICE_MB_TYPE.PNG)

表中，Pred_L0 表示使用 L0，即前向预测，Pred_L1 表示使用 L1，即后向预测，Bipred 表示双向预
测，Direct 表示直接预测模式。预测模式（mb_type,n）预测模式是 mb_type 的函数，n 是宏块的第
n 个分区。

### 帧内预测宏块

- **I_16x16**：整个16x16宏块使用单一预测模式，支持4种预测方向（垂直、水平、DC、平面）。
- **I_4x4**：将宏块划分为16个4x4子块，每个子块独立选择预测模式（9种方向模式）。
- **I_8x8**（仅High Profile支持）：将宏块划分为4个8x8子块，每个子块独立预测（9种方向模式）。
- 
...
CTU：最大的基本单位，固定大小为64X64 ，其实这样说不太准确，CTU的概念是包含的亮度和色度的块的，因为对于单独的亮度后者色度，只存在CTB的说法

CTB/CB 亮度或者色度块的单位，表示划分/不划分

若划分，则只能四叉树划分，最大64X64

TU:变换单元，作为依赖CU的子结构存在，其只支持四分划分。

PU：预测单元，PU划分依赖cu，是cu的子块，但是在一个cu内，TU和CU没有直接关系，TU可跨越多个PU

PU的划分依赖CU意味着，PU 是不能自己再向下划分的，必须是 以CU为父来划分PU

PU携带所有预测信息

帧内预测时候，PU携带方向等信息，帧内预测情形下，PU只支持2NX2N NXN



帧间预测的时候，PU携带划分方式，运动矢量预测等
...
NX4N + 3NX4N 即四分之一切一刀，不同方向加起来共有四种

帧内PU支持四分划分

![X265_PU.png](videocodeciamges/X265_PU.png)

## av1:

AV1 没有hevc中的PU，有类似于TU的变换单元

四叉树 + 二分(2种) + NX4N(2种) +  一边二分一边不分（T字）(4种) 最大128x128（superblock）

另外

NX4N不能用于 8X8和128X128的块
...
只有 PARTITION_SPLIT(四分划分)的时候才能继续划分，否则不能继续划分，像hevc中的CU划分PU逻辑

AV1中变换块的逻辑：

block划分为变换块的逻辑：见下图

![X265_PU.png](videocodeciamges/AV1_TX_BLOCK.png)![](videocodeciamges/AV1_TX_BLOCK2.png)

总结下来就是：

1：1的方形block ：只能四分

1：2或者2：1的矩形block ：分为两个方形

1：4或者4：1的矩形block ：分为两个2:1或者1：2

tx的划分可以有不同深度的不同方案，且最多只有两轮划分

对于宽高大于64的块，会有隐式划分为多个64X64，（这个逻辑看 residual()函数）

然后再继续划分TX块，如果小于等于64，则按照上面的逻辑直接划分

a：对于帧内Tx划分

所划分的子块必须一样大

例如一个32X16的可以划分为两个16X16,再继续划分的话就系要划分为四个8x8

另外，下图展示了tx多重划分的时候的编码顺序

![](videocodeciamges/AV1_TX_scan.png)

b:对于帧间TX划分

亮度：
帧间TX第一轮划分之后的子块，可以不互干扰的进行第二轮划分，所以帧间的块经过两轮划分之后，可以有不同大小的子块

如下图

![](videocodeciamges/AV1_TX_INTER.png)

色度：

  色度TX块的大小与色度编码块一样大，除非色度编码块宽高大于32。那么TX块的对应宽后者高就设为32
...
DC模式：上边一行左边一列加起来的平均值

planer模式:以H265的planer为例子：

![SAO](videocodeciamges/H265_INTRA_PLANER.PNG)

具体的计算步骤如下：

![SAO](videocodeciamges/hevc_planer.PNG)
...
![SAO](videocodeciamges/hevc_intra_map.png)

其中offset如下： 根据模式编号 确定offset值

![SAO](videocodeciamges/hevc_intra_direction_offset.png)

2.对填充出来的像素进行滤波：帧内预测的像素填充好之后，还需要对其进行滤波

3.进行预测：角度预测的情况下，会拿到填充好的用于帧内预测中的像素的两个点，并确定一个权重参数，使用如下公式计算最终的值：

![SAO](videocodeciamges/hevc_intra_omiga.png)

其中 ref 即为帧内预测准备的填充好的像素数组

对于 色度 DC模式 块大小小于 16x16 的情况，其最左边一列，最上边一行为平均值DCVALUE的四分之三

角度预测举例：

![SAO](videocodeciamges/hevc_direction_intra.png)
...
- 总结： dc + 3种 smoooth + PAETH_PRED + CFL+ 一种特殊的递归预测(后面详解)
  
   角度预测(分别是 V + H +DXXX 一共八种分别前后补充三个间隔3度的模式，
   一共是 7 X8  = 56 种，这里把V ， H 本身也计入角度模式)
  
  8个原始的角度分别为 ：45°、67°、90°、113°、135°、157°、180°、203
  
  在这些角度的基础上前后间隔3度各取3个角度
  
  PAETH_PRED是一种特殊的模式
  
  如第5图所示，根据其顶部（T）、左侧（L）和左上角（TL）的参考样本预测每个样本。在这些参考样本中，与（T + L – TL）的值最接近的那个值被选为预测样本。
  
  ![SAO](videocodeciamges/AV1_PAETHMODE.png)
  
  还有一个CFL模式 看后面16小节

三种smooth模式：

smooth模式它们通过距离加权计算预测值

![SAO](videocodeciamges/av1_intra_smooth.png)

它们的计算公式如下：

![SAO](videocodeciamges/av1_smooth_1.png)

其中w(x) w(y)表示 权重 ，在代码的

sm_weights_tx_XX系列数组中定义
...
recursiveIntraPrdiction  "递归滤波预测""

不属于帧内预测模式的一种，甚至不属于传统的帧内预测

在AV1中定义了五种递归滤波模式。每种模式指定了一组八个7-tap（7个系数的）滤波器。给定所选的递归内预测模式索引（0到4），当前块被划分为4×2子块的倍数。对于每个4×2子块，每个样本通过使用来自顶部和左侧块的七个相邻样本作为输入进行7-tap插值来预测，如图6所示。在每个4×2子块内不同坐标的样本应用不同的滤波器。这个预测过程在一个编码块内对4×2子块逐个执行，并且为每个4×2子块生成的预测样本可以用来预测下一个4×2子块。递归内预测的一个例子如图所示。

![SAO](videocodeciamges/av1_intra_recursive.png)

从上图来看，递归的过程是从左到右，从上到下，这样才不会出现source未进行重建的情况

# 4.熵编码

cabac解码代码查看 ffmpeg  :cabac_funtions.c

维护一个low 表示下限，range表示范围，按照cabac的定义，自适应二进制算术编码，会动态维护一个0的概率[LPS]和一个1的概率[MPS]

编码的时候，应该是range会不断减小，low整体呈增加趋势

解码的时候，range不断增大，low整体呈减小趋势

算术编码的计算复杂度主要体现在两个方面：
1） 概率的估计、更新
2） 划分子区间时的乘法运算： R = R × Px

H264x系列对这两个点都做了优化 ，简化计算复杂度

当前处理的字符为 MPS 时，区间递进只是子区间的长度发生改变，而
作为影响实际输出值的 L 却并没有变化。这个现象意味着如果输入流中连续出现大量 MPS，或者
MPS 对 LPS 的概率比非常高时，可以达到极高的压缩效果

当前处理的字符为 MPS 时，区间递进只是子区间的长度发生改变，而
作为影响实际输出值的 L 却并没有变化。这个现象意味着如果输入流中连续出现大量 MPS，或者
MPS 对 LPS 的概率比非常高时，可以达到极高的压缩效果

H26X中不同于法元素使用的上下文是什么？

cabac中概率计算需要知道上下文中MPS/LPS的数量，以264为例子，以slice作为一个cabac生命周期，但是实际上，作为一个生命周期，CABAC 也不是将它们作为整体来处理的，

仅提供码流的补充(refill过程),而概率的更新使用基于语法元素的的一些列预设上下文（当然，这些上下文会更新)，也就是说，同一组(H264)或者同一个语法元素(H265)的上下文才会相互影响影响后续的概率

每次更新概率的时候，应该根据之前出现的0和1来重新计算,实际计算中，为了降低计算复杂度，会使用一个预设的概率表：

量化0-0.5内的概率为64个值，如下，列出了 LPS 被量化后的概率值，这些值以σ 为编号，实线和虚线指示了概率的
刷新值。在处理当前字符 binval 时，概率的刷新有两个方向：一，如果当前字符是 LPS，则 Plps 变
大，在图上顺着虚线往左寻找；二，如果当前字符是 MPS，则 Plps 变小，在图上顺着实线往右寻找。

![SAO](videocodeciamges/cabac概率索引表.png)

对应的表格如下：

这个表格中σ表示当前概率，transIdxLps表示如果当前字符是LPS，则下一个选择的量化概率索引是多少

transIdxMps表示如果当前字符是MPS，则下一个选择的量化概率索引是多少

![SAO](videocodeciamges/cabac_1.png)

区间的更新也做了简化 进行查表:

![SAO](videocodeciamges/cabac_range.png)

另外 还有一种bypass 模式 ，0/1概率都是定为0.5，且不更新
...
多符号算术编码引擎（M-ary Arithmetic Coding Engine）用于语法元素的熵编码。每个语法元素与一个包含 M 个元素的字母表相关联，其中 M 可以是 2 到 16 之间的任意整数值。编码的输入是一个 M 进制符号，而上下文由一组 M 个概率组成。在编码/解析每个语法元素后，概率会更新，且概率以 15 位的累积分布函数（CDFs）表示。累积分布函数是一个包含 M 个 15 位整数的数组，定义如下：

                                                                                                    C=[c0​,c1​,…,cM−2​,215]

其中，cn​/32768 表示符号小于或等于 n 的概率。概率更新通过以下公式进行：
...
- α 是概率更新速率，根据符号已被解码的次数（最多 32 次）自适应调整；

- m 是 CDF 中的元素索引。

这种 α 的自适应机制允许在编码/解析语法元素的初始阶段更快地更新概率。多符号算术编码过程遵循传统的算术编码引擎设计，但只有高 9 位被输入到算术编码器/解码器中。
...
无wpp：类似HM，在分析一个帧之后，再做帧的滤波及帧的编码。  

全假wpp:只对分析内部并行，每一行一个线程，但滤波和编码过程没有流水的并行。  

半假wpp:即除熵编码以外流水并行,我们编码器的快速档就使用这种过程:每分析一个CTU后会滤波左上角的块,这样行与行之间即为流水级的并行关系。  

编码不能并行的原因是AV1标准不支持wpp语法,因此只能串行。  

快速AV1编码器需要做到半假wpp工作。  
。  
半真wpp:对于硬件编码器特别是266编码器,它的滤波如果做流水线并行,损失会比较大,所以硬件编码器就可以做半真wpp并行。  

其中,语法为了减少输出码流延迟会并行,但滤波过程为了提升压缩率就不会并行,分析过程也可以并行。  
。  
全真wpp:传统快速档的265编码器都会使用，分析滤波和编码一起流水线进行。  
...
1. **指定量化步长**：
   ...
   - 首先在帧头中传递一个语法元素 `base_q_idx`，用于指定亮度 AC 系数的量化步长。
   
   - `base_q_idx` 是一个 8 位固定长度码，取值范围为 [0, 255]。

2. **亮度 DC 系数的量化步长**：
   
   - 接着传递亮度 DC 系数相对于 `base_q_idx` 的差值（表示为 `DeltaQYDc`）。

3. **色度分量的量化步长**：
   
   - 如果存在多个颜色平面，则传递一个标志位 `diff_uv_delta`，指示是否对 Cb 和 Cr 颜色分量应用不同的量化索引值。
   
   - 如果 `diff_uv_delta` 为 0，则仅传递色度 DC 系数（表示为 `DeltaQUDc`）和 AC 系数（表示为 `DeltaQUAc`）相对于 `base_q_idx` 的差值。
   
   - 如果 `diff_uv_delta` 为 1，则分别传递 Cb 和 Cr 的 DC 系数（表示为 `DeltaQUDc` 和 `DeltaQVDc`）以及 AC 系数（表示为 `DeltaQUAc` 和 `DeltaQVAc`）相对于 `base_q_idx` 的差值。

4. **量化索引的计算**：
   
   - 将解码得到的 `DeltaQYDc`、`DeltaQUAc`、`DeltaQUDc`、`DeltaQVAc` 和 `DeltaQVDc` 值与 `base_q_idx` 相加，得到量化索引。
   
   - 这些量化索引会根据适当的查找表进一步映射到量化步长。

5. **量化步长的映射**：
   
   - 对于 8 位、10 位和 12 位内部比特深度，DC 系数的量化索引到量化步长的映射由 3×256 查找表 `Dc_Qlookup` 指定。
   
   - AC 系数的量化索引到量化步长的映射由 3×256 查找表 `Ac_Qlookup` 指定。
   
   - 图 18 和图 19 分别展示了 DC 和 AC 变换系数的量化索引（如 `base_q_idx`）到量化步长的映射关系。![loading-ag-23839](videocodeciamges/AV1_DCAC_QUAN.png)

10.去量化公式：给定量化步长 Qtstep，输入的量化系数 f 通过以下公式进一步去量化得到输出的去量化系数 F：

![loading-ag-23839](videocodeciamges/AV1量化.png)

   其中，sign 表示符号函数，% 表示取模运算，0xFF 表示十六进制的255。

    在解码端deNorm对应的东西叫做 dqDenom，解码反量化的值需要除以dqDenom，详情看源码

11.去量化常数 deNorm：deNorm 是一个根据变换块面积大小确定的常数，用于调整去量化系数的幅度。

12.deNorm 的选择：根据变换块的面积大小，不同变换块面积大小对应的 deNorm 值：

当变换块面积小于512个样本时，deNorm 为1。
当变换块面积为512或1024个样本时，deNorm 为2。
当变换块面积大于1024个样本时，deNorm 为4。
...
和H264是一毛一样的的，都是一维蝶形块速变换

但是支持 8X8 16X16 32X32 64X64,H264只有4X4的dct变换

一维变换

DCT  ADST   IDENTITY 这三种自由组合 两次一维变换，还包括ADST的flip版本

也就是说其实是 

DCT ADST/FLIPADST IDENTITY这四种进行自由组合
...
H265中预测是单独的 预测单元(PU)，以下均是基于预测单元
...
![H265mv预测](videocodeciamges/H265mv预测2.png)
...
![H265_PU_TIMRTEMPORAL](videocodeciamges/h265_TEMPORAL_mv.png)

时域候选列表中相邻已编码帧同位块的位置位于右下角H块，如果H块不存在，则使用C3来代替。时域候选列表最多只提供1个候选MV

![H265_PU_TIMRTEMPORAL](videocodeciamges/H265_PU_TIMRTEMPORAL.png)

### AMVP

AMVP 如果没有找到足够的候选mv 会补{0,0}，正如AMVP与merge的区别一样，AMVP有矢量残差，也只会编码矢量残差(MVD)
...
**参考帧差异处理**：若候选块的参考帧（Ref1）与当前块（Ref0）不同，根据时间距离缩放MV。

- 公式：缩放因子=候选块与其参考帧距离/当前块与其参考帧距离​

- 例如，Ref0距当前帧2帧，Ref1距4帧，则MV缩放为原值的0.5倍
...
av1的 mv 预测候选列表 ，经过空域，时域的筛选之后，再经过排序(基于mv 的权重)，留下最多四个，这剩下的四个就是 最终的候选列表

这个列表也叫作 dynamic reference list (DRL),然后再用于 dynamic MV prection 过程。
...
图示如下：另外，实际扫描mv的时候，时域扫描会在第三-四步中间

![H265_PU_TIMRTEMPORAL](videocodeciamges/AV1_mvp_spatial.png)

假设上图中所有小块都是8X8的

那么第一步的时候，扫描的是正上方这一行的四个8X8，需要注意的是，不是check整个8X8,而是这个8X8中靠近curent block的这两个4X4，

同理，第二步check 左边这四个8X8的收 也是每个8X8中靠近curent block的这两个4X4

第三步check右上角8X8 也只是靠近current的这个4X4，第四步同理

单参考帧的时候，会检测帧内空域邻近的块是否和当前块使用相同的参考帧，如果是，则会结合当前块的mv与邻近块的mv，作为最终mv

双参考帧同理

也就是说，对于这些空域候选块，只有他们与当前块使用相同的参考帧的时候，才会被考虑作为候选mv

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_sptaial_single_mv.png)

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_sptaial_compound_mv.png)

### 时域mv预测

参考帧的mv与参考帧的索引都需要存储

一帧中，对于每一个8X8 的块，如果有参考帧的mv穿过了当前这个mv，那么这个mv和其参考索引就会被存储起来

如下图，右手边的R1帧的MVref指向最左边的帧，穿过了current frame的一个8x8块，MVref就会被存储在和这个8x8关联的mv buffer中

按照如下顺序 扫描参考帧列表 ：LAST_FRAME, BWDREF_FRAME, ALTREF2_FRAME, ALTREF_FRAME, and LAST2_FRAME

（这个过程在 motion_field_estimation中）这里的 mv buffer 就是 MotionFieldMvs

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_temporal_linearprojection.png)

实际时域mv示例：

对于由图中B0,B1,B2,B3组成16X16块，按照顺序检查7个8X8块(看 temporal_scan函数)

从代码来看，

这里的逻辑是 中间区域，当前块宽高小于64，和当前块重叠的区域 会分成8X8的块来搜索，大于等于的64    的时候，则是16X16

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_temporal_mv_precdictor_chooser.png)

### Dynamic motion vector prediction

下面这些模式指示了如何从DRL队列中选取MV

//这四种是单参考帧的模式

    NEARESTMV : 始终使用最近的，也就是0下标的mv

    NEARMV  ： 使用 1，2，3下标的mv之一，

    GLOBALMV ：使用之前搜索的全局mv(GlobalMvs数组)

    NEWMV  ：使用 1，2，3下标的mv之一，同时还有mv残差

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_mv_mode_single.png)

//以下是双参考帧的模式

    NEAREST_NEARESTMV ：使用最近的0下标mv，无mv残差

    NEAR_NEARMV：使用 1，2，3下标的mv，无mv残差

    NEAREST_NEWMV  使用最近的0下标mv，第二个mv 有mv残差

    NEW_NEARESTMV 使用最近的0下标mv，第一个mv 有mv残差

    NEAR_NEWMV      

    NEW_NEARMV     使用 1，2，3下标的mv，第一个mv 有mv残差

    NEW_NEWMV     使用 1，2，3下标的mv，两个mv都 有mv残差

    GLOBAL_GLOBALMV 使用之前搜索的全局mv(GlobalMvs数组)

    

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_mv_compound_mode.png)

### Translational motion compensation

即平移运动补偿,仿射运动变换是平移运动变换的扩展，如figure15的右图，扩展了缩放和旋转

这种模式假块内的所有像素都是用同一个单mv，这个mv可以达到1/16像素精度

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_translation_mode_affine_mode.png)

### Warped motion compensation

这个模式使用affine 仿射模型(figure 15),

模型如下：

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_affine_formulation.png)
...
#### Global warped motion compensation

#### Local warped motion compensation

满足以下情况可以使用 Local warped motion 模式

1.当前块是单参考帧

2.宽或高大于等于8个像素

3.至少有一个相邻块和当前块使用相同的参考帧

如果当前块使用了局部扭曲运动（local warped motion），则通过最小化参考帧与基于当前块及其相邻块的运动矢量（MVs）的模型投影之间的均方误差来估计仿射模型参数。为了估计局部扭曲运动的参数，如果相邻块使用与当前块相同的参考帧，则收集相邻块中心像素及其在参考帧中对应像素的投影样本对。之后，通过在中心位置的一个或两个维度上移动四分之一像素来创建三个额外的样本。这些额外的样本也被视为投影样本对，以确保模型参数估计过程的稳定性。

用于推导运动参数的相邻块的运动矢量被称为运动样本。运动样本只能从使用与当前块相同参考帧的相邻块中选择。需要注意的是，扭曲运动预测模式仅适用于使用单一参考帧的块。这意味着运动样本只能从使用单向预测（单一参考帧）编码的块中选择。例如，在图16中，相邻块B0、B1和B2的运动矢量分别称为MV0、MV1和MV2。当前块使用参考帧Ref0进行单向预测。相邻块B0使用参考帧Ref0和Ref1进行复合预测；B1使用参考帧Ref0进行单向预测；B2使用参考帧Ref0和Ref2进行复合预测。由于只有B1与当前块使用相同的参考帧，因此MV1成为可用于推导当前块仿射运动参数的运动样本，而MV0和MV2则不能使用。

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_local_wrap.png)~~~~
...
OBMC（Overlapped Block Motion Compensation）是AV1视频编码标准中的一项关键技术，**旨在通过相邻块间的运动信息重叠补偿，减少预测误差和块效应，提升压缩效率与视觉质量**。
...
3.至少有一个相邻块使用inter 帧间编码

**当对当前块应用重叠块运动补偿（OBMC）时，首先使用与当前块相关联的运动矢量（MV）生成初始的帧间预测样本。然后，将当前块的帧间预测样本与使用上侧和左侧相邻块的运动矢量生成的帧间预测样本进行混合，以生成最终的预测样本。相邻块的运动矢量的最大数量受当前块大小的限制；在当前块的OBMC过程中，最多可以使用上侧和左侧块各四个运动矢量**。

figure17展示了相邻块的处理顺序示例，其中每个块中标记的值表示当前块和相邻块的运动矢量的处理顺序。具体来说，首先应用当前块的运动矢量生成帧间预测样本 p0​(x,y)。然后，应用块1的运动矢量生成预测样本 p1​(x,y)。之后，块0和块1之间重叠区域（在图17中以灰色标记,标准规定重叠区域宽/高为当前块的一半 ）的预测样本被计算为 p0​(x,y) 和 p1​(x,y) 的加权平均值。块2、块3和块4的运动矢量以相同的方式进一步应用并混合。

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_obmc.png)

### av1的compound帧间预测

公式：其中w(x,y)为权重

𝑃(𝑥, 𝑦) = (𝑤(𝑥, 𝑦) ∙ 𝑃0(𝑥, 𝑦) + (64 − 𝑤(𝑥, 𝑦)) ∙ 𝑃1(𝑥, 𝑦) + 32) ≫ 6

特别的，对于 SKIP模式

W(x,y)始终为32，也就是两边的权重一样

以下是几种权重选择模型：

#### Compound Wedge-based prediction

在基于楔形（Wedge-based）的复合预测模式中，对于每个块大小，预定义了一组16个二维权重数组。在每个数组中，权重的排列方式与预定义的楔形分割模式相对应。在每个楔形分割模式中，沿着特定的边缘方向和位置指定了两个楔形分区。对于位于其中一个楔形分区中的样本，权重主要设置为64；对于位于另一个楔形分区中的样本，权重主要设置为0。此外，沿着楔形分割边界，权重值设置为32

#### Difference-modulated compound prediction

这种模式的权重公式如下：其中mask_type从码流中读取

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_differtence_based_inter.png)
...
其中d0,d1代表当前帧到两个参考帧的距离

如果d0 < d1:

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_distance_base_1.png)

如果d1 < d0:

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_distance_based_2.png)

### AV1的Compound inter–intra 预测

这种模式 不是传统的 在inter帧中某个块使用intra 预测，而是结合inter预测和intra预测

𝑃(𝑥, 𝑦) = (𝑤(𝑥, 𝑦) ∙ 𝑃0(𝑥, 𝑦) + (64 − 𝑤(𝑥, 𝑦)) ∙ 𝑃1(𝑥, 𝑦) + 32) ≫ 6

还是这个公式，其中P0代表intra的预测样本，p1代表inter预测样本
...
#### Regular inter–intra prediction

以下是四种模式下的intra权重公式

![H265_PU_TIMRTEMPORAL](videocodeciamges/av1_interintra_model.png)

其中 WeightLUT是一个预设表(Ii_Weights_1d)，𝑠𝑖𝑧𝑒𝑆𝑐𝑎𝑙𝑒 = 128/max(𝑊, 𝐻),

#### Wedge-based inter–intra prediction

类似于 Compound Wedge-based prediction

它们的 wedge_sign 语法元素值不一样
...
UNIDIR_COMP_REFERENCE：两个参考帧来自同一个list，也就是要么都在当前帧前面，要么在后面，这种模式共有4种组合情况

BIDIR_COMP_REFERENCE：两个参考帧来自同一个list，一个当前帧前面，一个在后面，这种模式共有12种组合情况
...
- 确定后向的参考帧,将arf_stack中的POC最大的帧作为ALTREF_FRAME（这一帧一般是当前GOP的GPB）；如果alt_stack中还有其他帧，则第0帧作为BWDREF_FRAME，第1帧作为ALTREF2_FRAME。若arf_stack中只有1帧则将其作为BWDREF_FRAME。
...
在AV1标准中，参考帧的类型主要包括以下几种：

1. **LAST_FRAME**：最近解码的帧，常用于时间预测。

2. **LAST2_FRAME**：第二近解码的帧，用于更远的时间预测。

3. **LAST3_FRAME**：第三近解码的帧，提供更远的预测选项。

4. **GOLDEN_FRAME**：长期参考帧，用于较远时间的预测。

5. **BWDREF_FRAME**：后向参考帧，用于双向预测中的后向预测。

6. **ALTREF_FRAME**：替代参考帧，用于运动补偿预测。

7. **ALTREF2_FRAME**：第二个替代参考帧，提供额外的预测选项。

此外，**INTRA_FRAME**用于帧内预测，不参与帧间预测。

这些参考帧的灵活使用使得AV1能够更好地适应不同的视频内容，提高压缩效率。
...
SAO在解码端对重建图像的像素值进行调整，以补偿编码过程中引入的失真。它分为两种模式：

1. **Band Offset (BO)**：将像素值分成若干带（band），对每个带应用不同的偏移值。
...
1. **Edge Offset (EO)**：根据像素与其邻域像素的关系，对边缘像素应用不同的偏移值。

![SAO](videocodeciamges/SAO.png)

对于上图所示的任意一种模式，根据以下表格可以将去方块滤波之后的重构像素分为5个不同的类别

![SAO](videocodeciamges/sao_mode.jpeg)
...
1. **分类像素**：
   ...
   - **Band Offset (BO)**：将像素值范围划分为若干带（通常为32个带），每个带包含一定范围的像素值。根据像素值所属的带，决定应用的偏移值。
   
   - **Edge Offset (EO)**：根据像素与其邻域像素的关系，将像素分类为不同的边缘类型（如水平、垂直、对角线等）。常见的边缘类型有4种：水平、垂直、45度对角线和135度对角线。

2. **计算偏移值**：
   
   - 编码器会根据原始图像和重建图像的差异，计算每个类别（带或边缘类型）的最佳偏移值。这些偏移值会被编码并传输到解码端。

3. **应用偏移值**：
   
   - 解码端接收到偏移值后，根据像素的分类结果，对重建图像的像素值应用相应的偏移值，从而减少失真。 
...
严格来说 ，superres 是独立于编解码之外的步骤，编码的时候对原始的帧缩小分辨率，再进行编码

之后，解码的时候，先进行解码，然后再upscale，恢复为原始的图像
...
色度从亮度（Chroma from Luma，简称CfL）是一种仅应用于色度编码块的内预测模式。CfL预测模式通过一个线性模型，使用相应的重建亮度样本来推导色度预测样本。当相应的亮度和色度分辨率不同时，例如在4:2:0和4:2:2色度子采样格式中，需要在将重建的亮度样本输入CfL模式之前进行子采样。预测块是色度直流（DC）分量和缩放的亮度交流（AC）分量之和。一个块的DC分量由块的平均值组成，而AC分量是通过从块中移除DC分量来得到的。在CfL模式中，模型参数，例如应用于亮度AC分量的缩放因子，在编码过程中计算并信号化到比特流中。

CfL模式利用了亮度和色度之间的相关性，通过亮度信息来辅助色度的预测，从而提高编码效率，特别是在色度分辨率低于亮度分辨率的情况下。

![avfilters](videocodeciamges/AV1_CFL.png)
...
且块的尺寸大于等8X8 小于等64X64    

调色板模式和以下的intrabc都是为了提升屏幕录制场景性能而提出的特性
...
intraBC 是在loopfilter之前拷贝，inter prediction 是在loopfilter之后

为了优化硬件设计：在使用intrabc的时候，有以下限制

1.关闭换路后处理步骤，包括lp，lr，cdef

2.intabc仅在满足以下条件的时候可用(x0,y0,为当前superblock的左上角，（只有superblock可以用intabc？）)

- 垂直坐标 y 的值小于 y0；

- 水平坐标 x 的值小于 x0 + 2(y0 − y) − D，其中 D 表示从当前块左侧开始的水平方向像素数。在 AV1 中，D 被设置为超级块宽度的两倍，即 256 像素。

![avfilters](videocodeciamges/AV1_intraBC.png)
...
亮度 可以用 4,8,14抽头的滤波器，色度可以用4,6抽头滤波器

当边界两边的变换块的宽或者高大于16的时候，就可以用16抽头的滤波器

一般来说，去块效应滤波器使用的是低通滤波器

但是为了避免出现过度平滑，满足以下条件的时候，边界样本被识别为高方差，且不应用去块效应滤波器
...
(查看 adaptiveFilterStrength这个函数)

### cdef

Constrained directional enhancement filter(约束方向增强滤波器)

cdef分为两步

- **主滤波器**：沿着图像边缘方向进行滤波，目的是增强边缘的清晰度。（ Edge direction estimation）

- **次滤波器**：沿着与边缘方向成45度角的方向进行滤波，CDEF能够避免过度平滑，从而减少振铃效应和块效应，进一步平滑图像中的伪影 （Directional filter）

#### Edge direction estimation

cdef以8X8块为单位进行滤波，总共有8个方向，如下：

 ![avfilters](videocodeciamges/AV1_CDEF.PNG)

关于d的选择，有以下公式：

![avfilters](videocodeciamges/AV1_CDEF_1.PNG)                          式一

其中

![avfilters](videocodeciamges/av1_CDEF_2.PNG)         式二

对这条公式的详细解释如下：

![avfilters](videocodeciamges/AV1_CDEF_3.PNG)

所以结论就是 对 d = {1-7}带入式二，选择Sd最大的d最为最后选择的d。

#### Directional filter

补充 Edge direction estimation的一些信息，如下figure22：

其中蓝色点是被滤波的点，使用四抽头滤波器对其进行滤波，

当滤波强度(这个值指的是什么还不清楚)为偶数的时候，a = 2 ，b = 4;

当滤波强度为奇数的时候，a = 3 ，b = 3;

![avfilters](videocodeciamges/AV1_CDEF_4.PNG)

而 Directional filter是沿着和第一步的滤波器呈45度角的方向进行滤波，如figure23所示，可以根据d的值进行上下对照

CDEF的滤波公式为：

 ![avfilters](videocodeciamges/AV1_CDEF_5.PNG)

其中

- xi,j​ 和 yi,j​ 分别是CDEF滤波器的输入和输出重建样本值。

- 上标 p 和 s 分别表示主滤波器和次滤波器。

- wp 和 ws 是主滤波器和次滤波器的权重因子。
...
对于8位内容，主滤波器的强度 Sp 范围为0到15，次滤波器的强度 Ss 可以是0、1、2或4。对于亮度分量，D 的范围为3到6；对于色度分量，D 的范围为2到4。

CDEF是一种非线性滤波过程，用于防止在跨边缘应用滤波时产生过度模糊。它通过忽略与当前待滤波像素差异过大的像素来实现。当当前像素与其邻域像素 d 的差异在给定阈值内时，f(d,S,D) 等于 d，即保留该像素的贡献。否则，f(d,S,D) 等于0，即忽略该像素的贡献。具体来说，强度 S 决定了允许的最大差异，而阻尼 D 决定了忽略滤波器抽头的样本位置。

### Loop restoration filter

lr用于 64×64, 128×128, or 256×256 blocks

主要目的是提高主观质量，减少伪影

#### Separable symmetric Wiener filter

Wiener filter 由两个分离的对称 7抽头滤波器组成，一个垂直，一个水平，先垂直，再水平

在码流中，由于是对称的，只需要写三个参数

![avfilters](videocodeciamges/AV1_WIENER.PNG)

#### Self-guided filter

自引导滤波器（SGF）通过引导滤波生成两个初始滤波帧 X1​ 和 X2​，并使用最小均方方法计算权重参数 α 和 β，最终将退化图像 X 恢复为高质量图像 Xr​。滤波器的性能由半径参数 r 和噪声参数 e 控制，这些参数需要编码到比特流中以供解码器使用。该方法在图像恢复中能够有效保留细节并减少噪声

自引导滤波器（Self-guided Filters, SGFs）旨在获得两个初始滤波帧 X1​ 和 X2​。最终的恢复帧 Xr​ 通过退化样本与  退化样本和粗略恢复之间的差异的组合获得，公式如下：

![avfilters](videocodeciamges/AV1_SFG1.PNG)

在编码器端，α 和 β 使用最小均方（Least Mean Squares, LMS）计算：

![avfilters](videocodeciamges/AV1_SFG2.PNG)

其中，A={X1​−X,X2​−X}；b=y−x；y 是记录原始样本的向量。

X1​ 和 X2​ 通过引导滤波获得，滤波器由半径参数 r 和噪声参数 e 控制，其中较大的 r 值意味着更高的空间方差，较大的 e 值意味着更高的范围方差。X1​ 和 X2​ 可以分别用 {r1​,e1​} 和 {r2​,e2​} 描述。为了应用滤波器，编码器需要将 r1​,e1​,r2​,e2​,α,β 的值编码到比特流中。

生成初始滤波帧 X1​ 和 X2​ 的步骤如下：

1. 给定一对 (r,e)，分别用于生成 X1​ 和 X2​。

2. 计算输入样本在 (2r+1)×(2r+1) 窗口内的均值 μ 和方差 σ2，其中窗口以待滤波像素为中心。

3. 对于每个样本，滤波后的样本通过以下公式计算：
   
   ![avfilters](videocodeciamges/AV1_SFG3.PNG)

4. 使用滤波后的样本 x′，分别通过 (r1​,e1​) 和 (r2​,e2​) 生成 X1​ 和 X2​。

生成的 X1​ 和 X2​ 作为自引导滤波器（SGF）的输入。SGF 使用公式 (14) 和编码的 (α,β) 进行应用。
